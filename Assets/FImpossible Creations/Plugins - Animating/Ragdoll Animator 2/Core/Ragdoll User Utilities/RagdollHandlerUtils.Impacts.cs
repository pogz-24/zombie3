...
namespace FIMSpace.FProceduralAnimation
{
    public static partial class RagdollHandlerUtilities
    {
        ...
        /// <summary>
        /// Adding physical push impact to the single provided rigidbody object, at provided position, which can result in extra torque on rigidbody
        /// </summary>
        /// <param name="velocity"> World space direction velocity </param>
        /// <param name="forcePosition"> World space source hit position </param>
        /// <param name="duration"> Time in seconds, set zero to impact just once </param>
        public static void User_AddRigidbodyImpactAtPosition(this IRagdollAnimator2HandlerOwner iHandler, Rigidbody rigb, Vector3 velocity, Vector3 forcePosition, float duration, ForceMode forceMode = ForceMode.Impulse, float delay = 0f, int waitFixedFrames = 0)
        {
            var handler = iHandler.GetRagdollHandler;
            ...
            if (duration <= 0f) // No impact duration - add impulse once
            {
                if (delay > 0f || waitFixedFrames > 0)
                {
                    handler.Caller.StartCoroutine(handler._IE_CallAfter(delay, () => { ApplyLimbImpactAtPosition(rigb, velocity, forcePosition, forceMode); }, waitFixedFrames));
                    return;
                }
                else
                {
                    ApplyLimbImpactAtPosition(rigb, velocity, forcePosition, forceMode);
                }

                return;
            }

            handler.Caller.StartCoroutine(handler._IE_SetPhysicalImpactAtPosition(rigb, velocity, forcePosition, duration, forceMode, delay, waitFixedFrames));
        }


        /// <summary>
        /// Adding spherical explosion push impact to the single provided rigidbody object
        /// </summary>
        /// <param name="explosionForce"> Spherical explosion's force </param>
        /// <param name="forcePosition"> World space source hit position </param>
        /// <param name="explosionRadius"> Radius of the physics explosion range for the single rigidbody object </param>
        /// <param name="duration"> Time in seconds, set zero to impact just once </param>
        public static void User_AddRigidbodyExplosionForce(this IRagdollAnimator2HandlerOwner iHandler, Rigidbody rigb, float explosionForce, Vector3 explosionPosition, float explosionRadius, float upwardsModifier = 0f, float duration = 0f, ForceMode forceMode = ForceMode.Impulse, float delay = 0f, int waitFixedFrames = 0)
        {
            var handler = iHandler.GetRagdollHandler;
            ...
            if (duration <= 0f) // No impact duration - add impulse once
            {
                if (delay > 0f || waitFixedFrames > 0)
                {
                    handler.Caller.StartCoroutine(handler._IE_CallAfter(delay, () => { ApplyLimbExplosionForce(rigb, explosionForce, explosionPosition, explosionRadius, upwardsModifier, forceMode); }, waitFixedFrames));
                    return;
                }
                else
                {
                    ApplyLimbExplosionForce(rigb, explosionForce, explosionPosition, explosionRadius, upwardsModifier, forceMode);
                }

                return;
            }

            handler.Caller.StartCoroutine(handler._IE_SetExplosionImpact(rigb, explosionForce, explosionPosition, explosionRadius, duration, upwardsModifier, forceMode, delay, waitFixedFrames));
        }
        ...
        /// <summary>
        /// Adding physical push impact to whole chain bones rigidbodies, with force origin positioned in world space
        /// </summary>
        /// <param name="velocity"> World space direction velocity </param>
        /// <param name="position"> Origin position of the physical force to apply </param>
        /// <param name="duration"> Time in seconds, set zero to impact just once </param>
        public static void User_AddChainImpactAtPosition(this IRagdollAnimator2HandlerOwner iHandler, RagdollBonesChain chain, Vector3 velocity, Vector3 position, float duration, ForceMode forceMode = ForceMode.Impulse)
        {
            if (chain == null) return;

            if (duration <= 0f) // No impact duration - add impulse once
            {
                foreach (var bone in chain.BoneSetups) User_AddRigidbodyImpactAtPosition(iHandler, bone.GameRigidbody, velocity, position, duration, forceMode);
                return;
            }
            ...
            handler.Caller.StartCoroutine(handler._IE_SetChainPhysicalImpactAtPosition(chain, velocity, position, duration, forceMode));
        }
        ...
        /// <param name="position"> Origin position of the physical force to apply </param>
        /// <param name="duration"> Time in seconds, set zero to impact just once </param>
        public static void User_AddChainImpactAtPosition(this IRagdollAnimator2HandlerOwner iHandler, ERagdollChainType chain, Vector3 velocity, Vector3 position, float duration, ForceMode forceMode = ForceMode.Impulse)
        {
            User_AddChainImpactAtPosition(iHandler, iHandler.GetRagdollHandler.GetChain(chain), velocity, position, duration, forceMode);
        }
        ...
        public static void ApplyLimbImpact(Rigidbody rigidbody, Vector3 powerDirection, ForceMode forceMode = ForceMode.Impulse)
        {
            rigidbody.AddForce(powerDirection, forceMode);
        }

        /// <summary> Impulse impact on the rigidbody (AddForceAtPosition) </summary>
        public static void ApplyLimbImpactAtPosition(Rigidbody rigidbody, Vector3 powerDirection, Vector3 forcePosition, ForceMode forceMode = ForceMode.Impulse)
        {
            rigidbody.AddForceAtPosition(powerDirection, forcePosition, forceMode);
        }

        /// <summary> Spherical explosion impact on the rigidbody (AddExplosionForce) </summary>
        public static void ApplyLimbExplosionForce(Rigidbody rigidbody, float explosionForce, Vector3 explosionPosition, float explosionRadius, float upwardsModifier = 0f, ForceMode forceMode = ForceMode.Impulse)
        {
